...
/**
 * Class to manage options of MetisAPI
 * @author dinhvan
 */
public class MetisOptions {
    ...
    private int[] options = new int[METIS_NOPTIONS];

    /**
     * METIS_SetDefaultOptions
     * @return defaut options of METIS, set -1 to all
     */
    ...
    /**
     * @return array of options[METIS_NOPTIONS]
     */
    ...
    /**
     * options[METIS OPTION PTYPE] Specifies the partitioning method. 
     * Possible values are:<br>
     * METIS PTYPE RB Multilevel recursive bisectioning.<br>
     * METIS PTYPE KWAY Multilevel k-way partitioning.<br>
     */
    public int METIS_OPTION_PTYPE = METIS_PTYPE_RB;
    /**
     * options[METIS OPTION OBJTYPE] Specifies the type of objective. 
     * Possible values are:<br>
     * METIS OBJTYPE CUT Edge-cut minimization.<br>
     * METIS OBJTYPE VOL Total communication volume minimization.<br>
     */
     ...
     * values are:<br>
     * METIS CTYPE RM Random matching.<br>
     * METIS CTYPE SHEM Sorted heavy-edge matching.<br>
     */
     ...
     * values are:<br>
     * METIS IPTYPE GROW Grows a bisection using a greedy strategy.<br>
     * METIS IPTYPE RANDOM Computes a bisection at random followed by a
     * refinement.<br>
     * METIS IPTYPE EDGE Derives a separator from an edge cut.<br>
     * METIS IPTYPE NODE Grow a bisection using a greedy node-based strategy.<br>
     */
    public int METIS_OPTION_IPTYPE = METIS_IPTYPE_GROW;

    /**
     * options[METIS OPTION RTYPE]
     * Determines the algorithm used for refinement. Possible values are:<br>
     * METIS RTYPE FM FM-based cut refinement.<br>
     * METIS RTYPE GREEDY Greedy-based cut and volume refinement.<br>
     * METIS RTYPE SEP2SIDED Two-sided node FM refinement.<br>
     * METIS RTYPE SEP1SIDED One-sided node FM refinement.<br>
     */
     ...
     * obtained by a bit-wise OR of the following values.<br>
     * METIS DBG INFO (1) Prints various diagnostic messages.<br>
     * METIS DBG TIME (2) Performs timing analysis.<br>
     * METIS DBG COARSEN (4) Displays various statistics during coarsening.<br>
     * METIS DBG REFINE (8) Displays various statistics during refinement.<br>
     ...
     * METIS DBG SEPINFO (64) Displays information about
     * vertex separators.<br>
     * METIS DBG CONNINFO (128) Displays information related to the minimization
     * of subdomain connectivity.<br>
     * METIS DBG CONTIGINFO (256) Displays information related to the
     * elimination of connected components.<br>
     */
     ...
     * stage of the uncoarsening process. <br>Default is 10.
     */
     ...
     * communication volume. <br>Default is 1.
     */
     ...
     * distributions.<br>
     * 0 Performs a 2–hop matching.<br>
     ...
     * interface.<br>
     * 0 Does not explicitly minimize the maximum connectivity.<br>
     * 1 Explicitly minimize the maximum connectivity.<br>
     */
     ...
     * 0 Does not force contiguous partitions.<br>
     * 1 Forces contiguous partitions.<br>
     */
     ...
     * vertices that have identical adjacency lists.<br>
     * 0 Does not try to compress the graph.<br>
     * 1 Tries to compress the graph.<br>
     */
     ...
     * identified and ordered separately.<br>
     * 0 Does not identify the connected components.<br>
     * 1 Identifies the connected components.<br>
     */
     ...
     * If the specified value is x superior 0, then any vertices with a degree
     * greater than 0.1*x*(average degree) are removed from the graph, an
     * ordering of the rest of the vertices is computed, and an overall ordering
     * is computed by ordering the removed vertices at the end of the overall
     * ordering. For example if x = 40, and the average degree is 5, then the
     * algorithm will remove all vertices with degree greater than 20. The
     * vertices that are removed are ordered last (i.e., they are automatically
     * placed in the top-level separator). 
     * <br>Good values are often in the range of
     * 60 to 200 (i.e., 6 to 20 times more than the average). Default value is
     * 0, indicating that no vertices are removed. Used to control whether or
     * not the ordering algorithm should remove any vertices with high degree
     * (i.e., dense columns). This is particularly helpful for certain classes
     * of LP matrices, in which there a few vertices that are connected to many
     * other vertices. By removing these vertices prior to ordering, the quality
     * and the amount of time required to do the ordering improves.
     */
     ...
     * 1 Fortran-style numbering is assumed that starts from 1.<br>
     */
    ...
    /**
     * options[METIS_OPTION_PTYPE] Partitioning Schemes Multilevel recursive
     * bisectioning
     */
    public static final int METIS_PTYPE_RB = 0;
    /**
     * options[METIS_OPTION_PTYPE] Partitioning Schemes Multilevel k-way
     * partitioning
     */
    ...
    /**
     * options[METIS OPTION CTYPE] Coarsening Schemes Random matching
     */
    public static final int METIS_CTYPE_RM = 0;
    /**
     * options[METIS OPTION CTYPE] Coarsening Schemes Sorted heavy-edge matching
     */
    public static final int METIS_CTYPE_SHEM = 1;

    /**
     * options[METIS OPTION IPTYPE] Initial partitioning schemes Grows a
     * bisection using a greedy strategy
     */
    public static final int METIS_IPTYPE_GROW = 0;
    /**
     * options[METIS OPTION IPTYPE] Initial partitioning schemes Computes a
     * bisection at random followed by a refinement
     */
    public static final int METIS_IPTYPE_RANDOM = 1;
    /**
     * options[METIS OPTION IPTYPE] Initial partitioning schemes Derives a
     * separator from an edge cut
     */
    public static final int METIS_IPTYPE_EDGE = 2;
    /**
     * options[METIS OPTION IPTYPE] Initial partitioning schemes Grow a
     * bisection using a greedy node-based strategy
     */
    public static final int METIS_IPTYPE_NODE = 3;
    /**
     * options[METIS OPTION IPTYPE] Initial partitioning schemes
     */
    public static final int METIS_IPTYPE_METISRB = 4;

    /**
     * options[METIS OPTION RTYPE] Refinement schemes FM-based cut refinement
     */
    public static final int METIS_RTYPE_FM = 0;
    /**
     * options[METIS OPTION RTYPE] Refinement schemes Greedy-based cut and
     * volume refinement
     */
    public static final int METIS_RTYPE_GREEDY = 1;
    /**
     * options[METIS OPTION RTYPE] Refinement schemes Two-sided node FM
     * refinement
     */
    public static final int METIS_RTYPE_SEP2SIDED = 2;
    /**
     * options[METIS OPTION RTYPE] Refinement schemes One-sided node FM
     * refinement
     */
    public static final int METIS_RTYPE_SEP1SIDED = 3;

    /**
     * options[METIS OPTION DBGLVL] Debug Levels Shows various diagnostic
     * messages
     */
    public static final int METIS_DBG_INFO = 1;
    /**
     * options[METIS OPTION DBGLVL] Debug Levels Perform timing analysis
     */
    public static final int METIS_DBG_TIME = 2;
    /**
     * options[METIS OPTION DBGLVL] Debug Levels Show the coarsening progress
     */
    public static final int METIS_DBG_COARSEN = 4;
    /**
     * options[METIS OPTION DBGLVL] Debug Levels Show the refinement progress
     */
    public static final int METIS_DBG_REFINE = 8;
    /**
     * options[METIS OPTION DBGLVL] Debug Levels Show info on initial
     * partitioning
     */
    public static final int METIS_DBG_IPART = 16;
    /**
     * options[METIS OPTION DBGLVL] Debug Levels Show info on vertex moves
     * during refinement
     */
    public static final int METIS_DBG_MOVEINFO = 32;
    /**
     * options[METIS OPTION DBGLVL] Debug Levels Show info on vertex moves
     * during sep refinement
     */
    public static final int METIS_DBG_SEPINFO = 64;
    /**
     * options[METIS OPTION DBGLVL] Debug Levels Show info on minimization of
     * subdomain connectivity
     */
    public static final int METIS_DBG_CONNINFO = 128;
    /**
     * options[METIS OPTION DBGLVL] Debug Levels Show info on elimination of
     * connected components
     */
    public static final int METIS_DBG_CONTIGINFO = 256;
    /**
     * options[METIS OPTION DBGLVL] Debug Levels Show info related to wspace
     * allocation
     */
    public static final int METIS_DBG_MEMORY = 2048;

    /**
     * options[METIS OPTION OBJTYPE] Types of objectives Edge-cut minimization
     */
    public static final int METIS_OBJTYPE_CUT = 0;
    /**
     * options[METIS OPTION OBJTYPE] Types of objectives Total communication
     * volume minimization
     */
    public static final int METIS_OBJTYPE_VOL = 1;
    /**
     * options[METIS OPTION OBJTYPE] Types of objectives
     */
